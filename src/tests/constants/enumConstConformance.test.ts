...
const MIRRORS: {
  ...
  // participantRoles is a DEDICATED 1:1 image of ParticipantRoleEnum, so it is a mirror, not a bucket —
  // and unlike the five above it is HAND-AUTHORED rather than generated by scripts/generateEnumConstants,
  // which is precisely why it drifted: the const module carried 19 roles while the enum carried 17,
  // omitting SCOREKEEPER and TIMEKEEPER. `Participant.participantRole` is typed `ParticipantRoleUnion`,
  // so those two roles existed at runtime and could not be expressed by any type-safe consumer —
  // TMX's staff-role filter would not compile against its own runtime-valid list.
  {
    name: 'ParticipantRole',
    enum: T.ParticipantRoleEnum,
...
const ENUM_ONLY = [
  ...
  // ParticipantRoleEnum was listed here on the premise that it has no const-module twin. That was false:
  // `@Constants/participantRoles` is a dedicated 1:1 twin, and the exemption is what let the two diverge
  // unnoticed. It is registered as a MIRROR above.
