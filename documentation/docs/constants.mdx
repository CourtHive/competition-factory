...
```js
const { DIRECT_ACCEPTANCE, ALTERNATE, UNGROUPED, WILDCARD, QUALIFIER, LUCKY_LOSER, WITHDRAWN } =
...
**disciplineConstants** - `discipline` values

```js
const { TENNIS, BEACH_TENNIS, WHEELCHAIR_TENNIS } = factoryConstants.disciplineConstants;
```

`TENNIS`, `BEACH_TENNIS`, and `WHEELCHAIR_TENNIS` are the values of the
`event.discipline` field (`DisciplineUnion`).
