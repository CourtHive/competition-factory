...
```js
const { INTENNSE_STANDARD, TENNIS_STANDARD, MATCHUP, SET, HALF, SEGMENT, BETWEEN_GAMES, BETWEEN_POINTS, ANY } =
  factoryConstants.competitionFormatConstants;
```

`INTENNSE_STANDARD` / `TENNIS_STANDARD` name format profiles; the remainder are the
scopes a timing or interruption rule can apply to.
