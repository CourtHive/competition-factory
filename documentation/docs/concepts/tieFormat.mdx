...
When the resolved score source is `REPORTED`, the reported score and winningSide are preserved: a
recalculation will not overwrite them with a derivation from empty lines.
