...
See [tieFormat type definitions](../types/typedefs.mdx#tieformat)
...
- the read model projects `score_source: 'REPORTED'` on the TIE row, so a consumer can render "no line
  ...
  [`cast`](../governors/query-governor.md#cast)).

### Relationship to `disableAutoCalc`

[`disableTieAutoCalc`](../governors/matchup-governor.md) sets `disableAutoCalc` on a single TEAM `matchUp` and
...
- [tieFormat Governor](../governors/tie-format-governor.md) — methods for creating, modifying, deduplicating, and validating tieFormats
- [tieMatchUps](./tieMatchUp.mdx) — the individual matchUps within a TEAM matchUp
- [lineUp](./lineUp.mdx) — how participants are assigned to collection positions
