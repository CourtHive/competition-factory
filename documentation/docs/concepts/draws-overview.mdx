...
Draw structures are participant-type agnostic — the same elimination, round robin, and linked structures work whether participants are INDIVIDUAL, PAIR, or TEAM. The `eventType` determines what kind of participants flow through a draw, but the draw machinery itself (positioning, advancement, links) is identical in every case. See [Participant-Agnostic Logic](./participants#participant-agnostic-logic) for details.
