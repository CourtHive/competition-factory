...
## Participant-Type Agnostic

Draw structures are participant-type agnostic — the same elimination, round robin, and linked structures work whether participants are INDIVIDUAL, PAIR, or TEAM. The `eventType` determines what kind of participants flow through a draw, but the draw machinery itself (positioning, advancement, links) is identical in every case. See [Participant-Agnostic Logic](./participants.md#participant-agnostic-logic) for details.
...
- `drawType` - Type of draw structure (see [Draw Types](./draw-types.md) for full list)
...
- `qualifyingOnly` - Boolean - generate only qualifying structures with MAIN placeholder; see [Qualifying-First](#qualifying-first)
...
The `finishingPositions` array in each playoff group determines which group finishers advance to that bracket. See [Finishing Positions](./finishing-positions.md) for how these map to overall draw positions and ranking points.
...
#### Qualifying-First Draw {#qualifying-first}

Generate qualifying structures before the main draw exists. The MAIN structure is created as an empty placeholder and can be populated later.

```js
// Step 1: Generate qualifying-only draw
...
// At this point: qualifying structure has matchUps and positioned participants;
// MAIN structure exists but has 0 matchUps (placeholder).

// Step 2: Later, generate the main draw
const mainResult = tournamentEngine.generateDrawDefinition({
  drawId: drawDefinition.drawId, // existing draw
  drawSize: 32,
  drawType: 'SINGLE_ELIMINATION',
});
...
// MAIN is now populated; qualifying links are preserved.
```
...
- **[Draw Types](./draw-types.md)** - Complete list of supported draw types and structures
- **[Actions](./actions.mdx)** - Valid position and matchUp actions for draw management
