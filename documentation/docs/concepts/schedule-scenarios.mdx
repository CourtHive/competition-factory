...
- **Commit is a direct hand-off** — a scenario's `placements` array is, by construction, a [`bulkScheduleMatchUps`](../governors/schedule-governor.md#bulkschedulematchups) `matchUpDetails` payload.
...
- `removePriorValues` defaults to `true`, matching TMX grid-drop semantics (a re-dated matchUp sheds stale grid position — see [Scheduling Conflicts](./scheduling-conflicts.mdx)).
