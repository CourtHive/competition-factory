  ...
  events.forEach((event) => {
    event.drawDefinitions?.forEach((draw) => {
      draw.structures?.forEach((structure) => {
        const maxRound = structure.matchUps?.reduce((max, mu) => Math.max(max, mu.roundNumber), 0);
...
Schedule rounds in order that respects match dependencies. The factory's [`getMatchUpDependencies`](../governors/query-governor.md#getmatchupdependencies) method builds a full dependency graph that crosses structure boundaries (e.g., consolation draws, Round Robin → Playoff links) and can be used to validate profile ordering programmatically:
...
- **[Automated Scheduling](./automated-scheduling.md)** - Garman formula details
- **[Scheduling Policy](./scheduling-policy.mdx)** - Recovery times and daily limits
- **[Venues and Courts](./venues-courts.md)** - Venue configuration
