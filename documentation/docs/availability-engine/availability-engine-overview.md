---
title: Availability Engine Overview
---

The **AvailabilityEngine** is a pure JavaScript state machine that models court availability as continuous time-based capacity streams. Rather than tracking when courts are _available_, it tracks when they are _unavailable_ — blocks paint unavailable time onto courts, and availability rails and capacity curves are derived on demand. The engine is UI-agnostic, fully testable, and operates as a standalone class (not attached to the factory engine pattern).

## When to Use AvailabilityEngine vs scheduleGovernor

| Use Case                                        | Recommended            |
| ----------------------------------------------- | ---------------------- |
| Interactive visual scheduling (timeline editor) | **AvailabilityEngine** |
| Real-time availability queries                  | **AvailabilityEngine** |
| What-if simulation (preview before commit)      | **AvailabilityEngine** |
| Capacity analysis and utilization stats         | **AvailabilityEngine** |
| Conflict detection with pluggable evaluators    | **AvailabilityEngine** |
| Plan management (round assignment to days)      | **AvailabilityEngine** |
...
- **Standalone class** — Instantiate directly with `new AvailabilityEngine()`, no factory integration required
...
The AvailabilityEngine follows a three-layer architecture where blocks are the only canonical state and everything else is derived:

```text
┌──────────────────────────────────┐
│        AvailabilityEngine            │  Standalone class / facade
│  (block CRUD, plans, events,     │
│   availability, simulation)      │
├──────────────────────────────────┤
│      availabilityGovernor            │  Pure algorithm modules
...
:::note
Unlike the `ScoringEngine` which wraps `scoreGovernor` functions inside the factory engine pattern, the `AvailabilityEngine` is a **standalone class**. You instantiate it directly with `new AvailabilityEngine()` and pass a tournament record via `init()`. It does not register with the competition factory's engine infrastructure.
:::

## Constructor & Initialization

The `AvailabilityEngine` has no constructor parameters. All configuration is passed through `init()`:
