...
- **`buildDrawHierarchy` is removed** — a D3-era draw-rendering tree with no consumer anywhere in the ecosystem (measured across 1,809 source files in seven repos) and no documentation. Its implementation and tests are preserved verbatim outside this repository, so restoring it would be a copy rather than a rewrite — [open an issue](https://github.com/CourtHive/competition-factory/issues) if you need it. See [migration §6](./migration-7.0.0#6-builddrawhierarchy-is-removed).
