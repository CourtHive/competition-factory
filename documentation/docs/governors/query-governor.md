...
// rows: {
//   tournaments,            // one row: id, name, provider_id, dates, city, published (the tournament roll-up)
...
| `participantPublishRow`                    | Row builder for the `participant_publish` table (participant-list publication state); the `tournaments` row also carries an aggregate `published` flag (the tournament roll-up: a published event draw, order of play OR participants).                                                                                                                                                                                                                              |
| `getTournamentPublishStatus`               | Tournament-level publish status (its `orderOfPlay` / `participants` drive the order_of_play / participant_publish rows; the aggregate `tournaments.published` is the full roll-up from `getPublishState`); the tournament analog of `getEventPublishStatus`.                                                                                                                                                                                                         |
...
const {
  ...
  restrictAdHocRoundParticipants, // deprecated, no effect - AD_HOC: a participant already in the round is never offered
